...
const BarChart: React.FC<BarChartProps> = ({ transactions, period = 7, onChangePeriod }) => {
  ...
  // Group expenses by day for the selected period
  const dailyExpenses = daysArray.map(day => {
    const dayFormatted = formatDateForComparison(day);

    const dayExpenses = expenseTransactions.filter(transaction => {
      const txDate = transaction.date ? new Date(transaction.date) : null;
      return txDate && formatDateForComparison(txDate) === dayFormatted;
    });

    const totalAmount = dayExpenses.reduce((sum, tx) => sum + (Number(tx.amount) || 0), 0);
    ...
    return {
      ...
      amount: totalAmount,
      transactions: dayExpenses // Store the actual transactions for this day
    };
  });

  // Find the max value for scaling (ensure it's at least 1 to avoid division by zero)
  const maxTotal = Math.max(...dailyExpenses.map(day => day.amount), 1);
  ...
  return (
    <>
      ...
      <Box sx={{ display: 'flex', mt: 3, height: 240 }}>
        {/* Y-axis */}
                          ...
                          >
                            ...
                            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                              <Typography variant="caption" color="text.secondary">
                                {tx.notes?.substring(0, 20) || 'No description'}
